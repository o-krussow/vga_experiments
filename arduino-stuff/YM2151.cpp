...
//
#define		RD_HIGH		(PORTK = PORTK | 0x4)
#define		RD_LOW		(PORTK = PORTK & ~0x4)
#define		WR_HIGH		(PORTK = PORTK | 0x8)
#define		WR_LOW		(PORTK = PORTK & ~0x8)
#define		A0_HIGH		(PORTK = PORTK | 0x10)
#define		A0_LOW		(PORTK = PORTK & ~0x10)

#define		BUS_READ	DDRF=0x02;DDRK=0x3c;
#define		BUS_WRITE	DDRF=0xfe;DDRK=0x3f;
...
{
	uint8_t i,wi;
	volatile	uint8_t	*ddrF=&DDRF;
	volatile	uint8_t	*ddrK=&DDRK;
	volatile	uint8_t	*portF=&PORTF;
	volatile	uint8_t	*portK=&PORTK;

	if(last_write_addr != 0x20){
		// addr 0x20へのアクセスの後busyフラグが落ちなくなる病 '86の石だと発生
		// 他のレジスタに書くまで落ちないので、強引だが0x20アクセス後ならチェックしない
		*ddrF &= ~(_BV(2) | _BV(3) | _BV(4) | _BV(5) | _BV(6) | _BV(7));
		*ddrK &= ~(_BV(0) | _BV(1));
		wait(8);
		A0_LOW;
		wait(4);
	...
	*ddrK |= (_BV(0) | _BV(1));
	wait(8);
	A0_LOW;
	*portF = (addr << 2) | (*portF & 0x03);
	*portK = (addr >> 6) | (*portK & 0xfc);
	wait(4);
	...
	A0_HIGH;
	wait(2);
	*portF = (data << 2) | (*portF & 0x03);
	*portK = (data >> 6) | (*portK & 0xfc);
